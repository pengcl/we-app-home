...
Page({
	...
	data: {
		...
		ProjectCommunity_Gid: "3B3C090F-3B19-496C-92B2-32BA4F5678F7",
		windowHeight: 0,
		// hasVR: true,
		currHuXingVR: "",
		scrollintoview: "",
	...
	onLoad: function (options) {
		...
		if (app.globalData.appIsOnload == false) {
			...
			request.MainGetRequest("/Page/HZ_index", {}, function (
				res
			) {
				_this.setData({
					pageData: res.data,
	...
	YuYue_tap: function (e) {
		...
		let WantTypeID = this.data.WantTypeID;
		let WantType = this.data.WantType[WantTypeID];
	...
	map_tap: function (e) {
		var lat = 30.238003,
			lng = 120.190591,
			vname = "新世界中国在杭州",
			vaddress = "浙江省杭州高上城区秋涛路 婺江路地铁站-B口";
		wx.openLocation({
			type: "gcj02",
			latitude: lat,
			longitude: lng,
    ...
    this.setData({
      WantTypeID: e.detail.value//把当前的触摸的索引给expandTypeID
    })
  },
