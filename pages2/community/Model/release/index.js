 ...
 click3: function () {
	...
	onLoad: function (options) {
		...
		if (app.globalData.appIsOnload == false) {
			var data = {project_gid : options.scene};
			app.appIsOnloadCallback = () => {
				request.postRequest("/PageView/Project_index",	data,
					function (res) {
						if(res != null){
							_this.setData({
								ProjectCommunity_Name :res.ProjectName,
								ProjectCommunity_Gid :res.ProjectGID,
								xxInfo : res.XinxiData,
								dzInfo : res.DizhiData,
								fmInfo : res.FengmianData,
								xmInfo : res.XiangmuData,
								zfInfo : res.ZhuanfaData,
								lsInfo : res.LoushuData,
								ttInfo : res.ToutiaoData
							});
							_this.initSalers();
						}	
					},
